...
const scrollCallback = (startIndex: number, endIndex: number) => {
  const posY = scrollContainer.value ? scrollContainer.value.scrollTop : 0
  emits('scroll', startIndex, posY)
}

// 滚动到指定y位置
function scrollToPos(val: number) {
  if (!scrollContainer.value)
    return

  if (scrollContainer.value.scrollTop === val)
    return
...
<template>
  <div ref="scrollContainer" class="scroll-container" @scroll.passive="handleScroll">
    <ul class="scroll-container__list" :style="containerStyle">
    ...
    <div v-show="locked" class="scroll-container__bottom">
      ...
      <div v-else class="loading-state cursor" @click="handleRefresh">
        <span class="icon">
          <svg t="1651893386303" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
